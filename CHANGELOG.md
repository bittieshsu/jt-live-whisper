...
**新的辨識模型：Qwen3-ASR（實驗選項，需 GPU 伺服器）**

離線處理錄音檔時可以選 `qwen3-asr-0.6b`（命令列 `-m qwen3-asr-0.6b`、互動選單、WebUI 都有）。
用有標準答案的錄音、與現行 large-v3-turbo 同條件實測：

| 測試 | large-v3-turbo | Qwen3-ASR 0.6B |
|---|---:|---:|
| 中文真實會議 20 場（5~7 人）字錯率 | 28.78% | **15.75%** |
| 中英夾雜（中文為主）：英文詞找回比例 | 34.7% | **74.9%** |
| 低音量錄音字錯率 | 20.67% | **14.33%** |
| 韓文長檔字錯率 | 13.35% | **3.54%** |
| 日文長檔字錯率 | **6.97%** | 8.38% |

- **限制**：只支援離線處理、中文／英文／韓文單向模式。日文長檔實測較差、台語遠不如 Breeze-ASR-26，所以不開放；
  即時字幕、雙向模式照舊用 Whisper。不適用時會說明原因並改用推薦模型，選單與 WebUI 也不會讓你選到
- **只在 GPU 伺服器執行**：需要另外在伺服器建一個獨立環境（SOP「GPU 伺服器的 Qwen3-ASR」有指令），
  常駐約 7 GB 顯示記憶體。沒有安裝的伺服器完全不受影響，選單不會出現這個選項
- 時間戳：Qwen3-ASR 本身不給時間，伺服器先依語音活動切段，再用 Qwen3-ForcedAligner 對出每個字的時間後切句，
  字幕與時間逐字稿照常產生；只有「嗯」之類語氣詞的片段會丟掉（靜音時模型會吐這個）
- **出錯時的處理**（都在 GPU 伺服器實測過）：
  - 處理途中 Qwen3-ASR 失敗 → 自動改用 GPU 伺服器的 large-v3-turbo 重跑，不會退到本機 CPU
  - 伺服器上的 Qwen3-ASR 意外結束 → 30 秒後自動重啟，一小時內最多 3 次，超過就停用並在 `/health` 寫明原因；
    它底下的 vLLM 子行程會一起收掉（實測不收的話每次殘留約 5 GB 顯示記憶體）
  - 辨識服務停止、被強制結束、或用遠端更新換版 → Qwen3-ASR 子行程一起結束，不會殘留佔用顯示記憶體
  - 埠號已被佔用（例如同一台跑了兩個服務）→ 不搶、停用 Qwen3-ASR 並說明；第一次啟動要下載模型時不會被誤判成啟動失敗

**講者辨識：預備改用 NVIDIA Nemotron 3 Diarization（這一版還不會啟用）**

- 同一批真實辨識段落實測：段落標錯講者 中文 20 場 18.52% → 3.07%、英文 16 場 12.31% → 4.65%，人數判對 2/20 → 17/20
- 它需要 transformers 5.18（尚未推出），**沒有的環境一律沿用現行方法，結果與先前完全相同**（已用同一份錄音逐段比對）
- 新增 `--diarize-engine auto|nemotron|legacy`；啟用後指定人數會改為「上限」，並在超過 8 人時自動改用現行方法

**其他**
- WebUI 模型選單支援後端給的使用限制（例：僅限錄音檔、需 GPU 伺服器），不適用的選項會停用並寫出原因
- 文件更正：網站「安裝與使用」的摘要模型預設值還寫 gpt-oss:120b（實際為 qwen3.8:27b）、摘要模型「推薦 120B 以上」
  （實測 27B 較好）、離線選單「9 種模式」（實際 15 種）；README 的 `-m` 列出了不存在的 medium／medium.en

### v2.22.3 (2026-09-25)

**伺服器版的安全預設**

- **第一次安裝時同時產生唯讀密碼**：先前只產生管理密碼，唯讀密碼沒設時，同網段（或允許清單內）
  任何人都能看畫面、列出錄音、讀逐字稿與摘要。現在全新安裝會產生管理密碼與唯讀密碼，各印出一次。
  **已經設過管理密碼的既有部署一律不動**（升級不改變既有行為），只會提醒
- **WebUI 啟動時的安全提醒**：沒有唯讀密碼（或沒有管理密碼）時，在啟動訊息與系統記錄裡說明現況與設定方式
- **服務帳號不再意外成為 root**：先前用「執行安裝的帳號」，用 root 或 `sudo` 安裝時 WebUI 就以 root 執行——
  任何一個 WebUI 漏洞都等於整台主機的最高權限。現在依序用 `sudo` 的原始帳號、安裝資料夾的擁有者；
  兩者都是 root 才用 root，並出現警告

**文件**
- SOP 補上「三種部署方式」（單機、本機＋GPU 伺服器、伺服器模式）與伺服器模式的實測規格
  （搭配 GPU 伺服器時 2 vCPU、4 GB 記憶體、30 GB 磁碟；37 分鐘會議記憶體峰值約 0.4 GB）
- SOP「REST API」一節更正：這組 API 不含在公開發行內容，`install.sh --server` 不會安裝
- README、網站「安裝與使用」同步更新伺服器模式的說明
